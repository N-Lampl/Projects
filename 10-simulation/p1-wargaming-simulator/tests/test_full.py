...
def test_symmetric_scenario_is_fair():
    ...
    mc = monte_carlo("meeting_engagement", "aggressive", "aggressive", n=3000)
...
def test_sensitivity_is_monotonic_in_red_count():
    ...
    sw = sensitivity_sweep(
        "meeting_engagement", "red_count", [1, 2, 3], "aggressive", "defensive", n=1000
    )
    rates = sw["winrate"]
...
def test_ab_delta_is_stable_under_common_random_numbers():
    ...
    ab = compare(
        "combined_arms", ("aggressive", "defensive"), ("payoff", "defensive"), n=2000
    )
