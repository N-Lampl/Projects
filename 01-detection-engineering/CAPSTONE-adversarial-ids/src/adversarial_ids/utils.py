...
def set_seed(seed: int = SEED) -> None:
    ...
    scikit-learn/numpy, so we never hard-require torch just to set a seed.
    """
...
def shared_pipeline_src() -> Path:
    ...
    The capstone deliberately does NOT vendor the pipeline -- it imports the
    shared library by path so ``p1-nids-baseline`` and this adversarial-IDS
    capstone share exactly one leak-free preprocessing + model implementation.
    """
    here = Path(__file__).resolve()
    # src/adversarial_ids/utils.py -> project root is parents[2]
